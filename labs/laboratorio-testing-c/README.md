...
> R:$2800 (350x2 = 700, 200x3 = 600, 1500x1 = 1500; total = 2800).
...
> R: carrito_init inicializa el contador cantidad = 0 del carrito. Es fundamental llamarla porque en C las variables locales contienen valores basura de memoria si no se inicializan explícitamente.carrito_init inicializa el contador cantidad = 0 del carrito. Es fundamental llamarla porque en C las variables locales contienen valores basura de memoria si no se inicializan explícitamente.
...
> R: Pasa (da OK), porque al tener cantidad 1, el precio unitario coincide con el subtotal ($350 x 1 = $350) y el bug no se manifiesta.
...
> R: En carrito_total, la línea total += c->items[i].precio; solo suma el precio unitario ignorando la cantidad del producto.

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Debe multiplicar el precio unitario por la cantidad: total += c->items[i].precio * c->items[i].cantidad;
...
> R:El test falla. El 5to intento de carrito_agregar devuelve 1 en lugar de 0.
...
> R:No hay líneas con #####. Todas las líneas y ramas condicionales fueron alcanzadas por la combinación de tests unitarios y de integración.
...
> R:Un test unitario verifica una función de forma aislada, mientras que uno de integración evalúa cómo interactúan varios módulos/funciones en conjunto. El bug de carrito_total fue detectado primero por el test unitario test_total_con_cantidad.
...
> R:C no realiza control de límites de memoria en tiempo de ejecución (bounds checking); escribir fuera del array corrompe datos adyacentes en memoria y genera comportamiento indefinido. Lenguajes como Java o Python validan los índices automáticamente y lanzan excepciones (IndexError/IndexOutOfBoundsException) impidiendo la corrupción.
...
> R:Escribir tests permite una validación automatizada, reproducible y objetiva contra casos borde, además de prevenir regresiones al modificar el código a futuro.
...
> R:Con cantidad = 1, la operación precio * 1 da el mismo valor que precio solo. Al no multiplicar por la cantidad, el código arrojó por coincidencia aritmética el resultado esperado para ese caso puntual.
